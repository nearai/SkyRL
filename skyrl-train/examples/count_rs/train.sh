...
DATA_DIR="$HOME/data/count_rs"
NUM_GPUS=8

uv run --isolated --extra vllm -m examples.count_rs.main_count_rs \
  ...
  environment.env_class=count_rs \
  generator.n_samples_per_prompt=5 \
  generator.gpu_memory_utilization=0.8 \
  trainer.logger="wandb" \
  trainer.project_name="count_rs" \
  trainer.run_name="count_rs_test" \
  $@
